...
class LinkInventSampleModel(BaseAction):
    ...
    def _randomize_warheads(self, warhead_pair_list: List[str]):
        ...
        for warhead_pair in warhead_pair_list:
            ...
            sub_mol = self._conversions.smile_to_mol(warhead_list[1])
            if not warhead_list[1].startswith('[*]'):
                raise ValueError("Input string must start with '[*]'.")
            if 'F' not in warhead_list[1]:
                warhead_list[1] = warhead_list[1][:3] + 'CFF' + warhead_list[1][3:]
                cleaned_substrings = re.split(r'FF', warhead_list[1])

                cleaned_substrings[0] = cleaned_substrings[0] + 'F'
                cleaned_substrings[1] = "F" + cleaned_substrings[1]
                cleaned_substrings[0] = cleaned_substrings[0].replace('[','').replace(']','')
            
                cleaned_substrings[1] = cleaned_substrings[1].replace('[','').replace(']','')
            elif 'F' in warhead_list[1]:
                warhead_list[1] = warhead_list[1][:3] + 'CClCl' + warhead_list[1][3:]
                cleaned_substrings = re.split(r'ClCl', warhead_list[1])

                cleaned_substrings[0] = cleaned_substrings[0] + 'Cl'
                cleaned_substrings[1] = "Cl" + cleaned_substrings[1]
                cleaned_substrings[0] = cleaned_substrings[0].replace('[','').replace(']','')
            
                cleaned_substrings[1] = cleaned_substrings[1].replace('[','').replace(']','')
            warhead_mol_list = [self._conversions.smile_to_mol(warhead) for warhead in cleaned_substrings]
