...
class LocalBondLinkReinforcementLogger(BaseReinforcementLogger):
    ...
    def _tensorboard_report(self, step, score_summary: FinalSummary, agent_likelihood, prior_likelihood,
        ...
        if step % 1 == 0:
            smiles_macro=[]
            for smi in score_summary.scored_smiles:
                ...
                if len(indexs_Cl) == 2 and len(indexs_F) < 2:
                    ...
                    smiles_macro.append('cccnfnf')

            self._log_out_smiles_sample(smiles_macro, score_summary.total_score, step, score_summary)
